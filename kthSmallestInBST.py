...
def buildTree(t):
    if t is not None:
        tree = Tree(t['value'])
        tree.left = buildTree(t['left'])
        tree.right = buildTree(t['right'])
        return tree

def readTreeAsList(t):
    if t is not None:
        z = ""
        z+= str( t.value ) + ","
        if t.left is not None:
            z+= readTreeAsList(t.left )+","
        if t.right is not None:
            z+= readTreeAsList(t.right )+","
        return str ( z )
...
def kthSmallestInBST(t, k):
    ...
    # submit
    raw_read = readTreeAsList(tree)
    vals = [ int(v) for v in raw_read.split(',') if len(v)>0 ]
